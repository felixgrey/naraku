...
function () {
  ...
  _createClass(DataHub, [{
    ...
    value: function loading(list) {
      ...
      return [].concat(list).reduce(function (a, b) {
        return a || _this35.status(b) === 'loading';
      }, false);
    }
  }, {
    key: "error",
    value: function error(list) {
      ...
      return [].concat(list).reduce(function (a, b) {
        return a || _this36.status(b) === 'error';
      }, false);
    }
  }, {
    key: "locked",
    value: function locked(list) {
      ...
      return [].concat(list).reduce(function (a, b) {
        return a || _this37.status(b) === 'locked';
      }, false);
    }
  }, {
    key: "lock",
    value: function lock(names) {
      ...
      names.forEach(function (name) {
        if (_this38.status(name) !== 'locked') {
          oldStatus.push({
            name: name,
            old: _this38.status(name)
          });

          _this38.status(name, 'locked');
        }
      });
      return function () {
        if (!unlock) {
    ...
    value: function submit(name) {
      ...
      var afterSubmit = function afterSubmit(data) {
        ...
        [].concat(refresh).forEach(function (refreshName) {
          _this39.refresh(refreshName);
        });
      };

      return new Promise(function (resolve, reject) {
        _this39._controller.once('$fetchEnd:' + name, function (resultData) {
          if (_this39.status(name) === 'error') {
            _this39.set(name, []);
    ...
    value: function doFetch(action, name) {
      ...
      this._lagFetchTimeoutIndex[name] = setTimeout(function () {
        ...
        if (_this40.status(name) === 'loading') {
          (0, _Utils.errorLog)("".concat(name, " can not be fetched when loading"));
          return;
        }

        _this40.status(name, 'loading');

        param = (0, _Utils.snapshot)(param);
        Promise.resolve(param).then(function (param) {
          return _this40._controller.run('beforeFetcher', param, name, _this40);
        }).then(function (param) {
          return DataHub.dh._controller.run('beforeFetcher', param, name, _this40);
        }).then(function (newParam) {
          if (newParam === _Utils.stopRun || _this40._invalid) {
            return Promise.reject(_Utils.stopRun);
          }

          var submitData = (0, _Utils.snapshot)(_this40.get(name));
          ...
          return DataHub.dh._controller.run('afterFetcher', result, name, _this40);
        }).then(function (result2) {
          return _this40._controller.run('afterFetcher', result2, name, _this40);
          ...
          _this40.set(name, newResult);

          _this40.emit('$fetchEnd', newResult);

          _this40.emit('$fetchEnd:' + name, newResult);
        }).catch(function (e) {
          if (_this40._invalid) {
            return;
          }
          ...
          if (e === _Utils.stopRun) {
            _this40.status(name, 'set');
          } else {
            (0, _Utils.errorLog)(e);

            if (!_this40._data[name]) {
              _this40.set(name, []);
            }

            _this40.status(name, 'error');

            _this40.emit('$error', e);

            _this40.emit('$error:' + name, e);

            _this40.emit('$fetchEnd', e);

            _this40.emit('$fetchEnd:' + name, e);
          }
        });
      }, lagTime);
    }
