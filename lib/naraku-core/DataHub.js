  ...
  _createClass(DataHub, [{
    ...
    value: function doFetch(action, name) {
      ...
      this._lagFetchTimeoutIndex[name] = setTimeout(function () {
        ...
        if (_this41.status(name) === 'loading') {
          (0, _Utils.errorLog)("".concat(name, " can not be fetched when loading"));
          return;
        }

        _this41.status(name, 'loading');

        param = (0, _Utils.snapshot)(param);
        Promise.resolve(param).then(function (param) {
          return _this41._controller.run('beforeFetcher', param, name, _this41);
        }).then(function (param) {
          return DataHub.dh._controller.run('beforeFetcher', param, name, _this41);
        }).then(function (newParam) {
          if (newParam === _Utils.stopRun || _this41._invalid) {
            return Promise.reject(_Utils.stopRun);
          }

          var submitData = (0, _Utils.snapshot)(_this41.get(name));
          ...
          return DataHub.dh._controller.run('afterFetcher', result, name, _this41);
        }).then(function (result2) {
          return _this41._controller.run('afterFetcher', result2, name, _this41);
          ...
          _this41.set(name, newResult);

          _this41.emit('$fetchEnd', newResult);

          _this41.emit('$fetchEnd:' + name, newResult);
        }).catch(function (e) {
          if (_this41._invalid) {
            return;
          }
          ...
          if (e === _Utils.stopRun) {
            _this41.status(name, 'set');
          } else {
            (0, _Utils.errorLog)(e);

            if (!_this41._data[name]) {
              _this41.set(name, []);
            }

            _this41.status(name, 'error');

            _this41.emit('$error', e);

            _this41.emit('$error:' + name, e);

            _this41.emit('$fetchEnd', e);

            _this41.emit('$fetchEnd:' + name, e);
          }
        });
      }, lagTime);
    }
