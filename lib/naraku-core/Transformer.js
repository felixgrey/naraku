...
const _aggregates = {
  ...
  'median': (_ref3) => {
    let field = _ref3.field,
        ...
        data = _ref3.data;
    const itemValue = (0, _Utils.noValue)(item[field]) ? defaultValue : item[field];
    const tempValueKey = "_currentMedianValue.".concat(field);
  ...
  // 聚合条数
  'count': (_ref6) => {
    let keyCounts = _ref6.keyCounts;
    return keyCounts + 1;
  },
  // 字符串连接
  'join': (_ref7) => {
    let field = _ref7.field,
        value = _ref7.value,
        item = _ref7.item,
        option = _ref7.option,
...
function traceObject(item, valuePath) {
  let display = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
  let traceCallback = arguments.length > 3 ? arguments[3] : undefined;

  let _midResult = arguments.length > 4 ? arguments[4] : undefined;

  let countField = arguments.length > 5 ? arguments[5] : undefined;

  let _currentPath = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : '';
  ...
  const surplusFields = valuePath.replace(_currentPath, '').replace(/^\./g, '').split('.');
  const field = surplusFields.shift();
  ...
  if (typeof fieldValue === 'object' && surplusFields.length) {
...
let TransformProcess = (_class = class TransformProcess {
  ...
  fromModel() {
    ...
    let outPutList = [];
    const fromList = branch.map(a => a.from);
    const buildSeed = originField === false ? () => ({}) : item => ({
      [originField]: item
    });
    list.forEach(item => {
      // item, valuePath, display, traceCallback, _midResult, countField, _currentPath
      const expendItems = traceObject(item, main, true, function (fieldPath, v, mid) {
        fromList.forEach((from, index) => {
          if (from.indexOf(fieldPath) === 0) {
            const field = from.replace(fieldPath, '').replace(/^\./g, '');

            if (field !== '') {
              const _branch$index = branch[index],
                    to = _branch$index.to,
                    leaf = _branch$index.leaf,
                    set = _branch$index.set;

              const _traceObject = traceObject(v, field, false),
