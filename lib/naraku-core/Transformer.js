...
function traceObject(item, valuePath) {
  let _currentPath = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';

  let _midResult = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};

  let countField = arguments.length > 4 ? arguments[4] : undefined;
  let display = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : true;
  ...
  if (Array.isArray(fieldValue) && display) {
    ...
    fieldValue.forEach((v, i) => {
      ...
      newMidResultList = newMidResultList.concat(traceObject(v, valuePath, fieldPath, clone, countField, display));
    });
...
let TransformProcess = (_class = class TransformProcess {
  ...
  fromModel() {
    ...
    const fromList = [];
    const toList = [];
    const setList = branch.map((_ref11) => {
      let from = _ref11.from,
          to = _ref11.to,
          set = _ref11.set;
      fromList.push(from);
      toList.push(to);
    ...
    list.forEach(item => {
      const mergedItem = {};
      branch.forEach((_ref12) => {
        let from = _ref12.from,
            to = _ref12.to,
            set = _ref12.set;

        const _traceObject = traceObject(item, from, '', {}, countField, false),
      ...
      expendItems.forEach(item2 => {
        Object.assign(item2, (0, _Utils.snapshot)(mergedItem));
      });
