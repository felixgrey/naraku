...
function groupStrToConfig(config) {
  ...
  var valueFields = valueFieldStr.replace(/^\s+|\s+$/g, '').split(',').map(function (field) {
    ...
    var _ref9 = field.indexOf('@') !== -1 ? field.split('@') : field.split(':'),
        _ref10 = _slicedToArray(_ref9, 2),
        fieldName = _ref10[0],
        aggregateType = _ref10[1];
  ...
  _createClass(TransformProcess, [{
    ...
    value: function fromObject() {
      ...
      this.data = Object.keys(obj).map(function (_k) {
        ...
        return _ref11 = {}, _defineProperty(_ref11, key, _k), _defineProperty(_ref11, value, obj[_k]), _ref11;
      });
    }
  }, {
    key: "operate",
    ...
    key: "toDisplayFields",
    value: function toDisplayFields(grouped, keyField, valueField, nameField) {
      if ((0, _Utils.noValue)(grouped) || (0, _Utils.noValue)(keyField) || (0, _Utils.noValue)(valueField)) {
        return;
      }

      var tempField = 'field_' + Date.now();

      if ((0, _Utils.noValue)(nameField)) {
        nameField = tempField;
      }

      this.toGrouped("".concat(grouped, " => ").concat(tempField, "@nothing"));
      var allFields = new Set();
      var code_name = {};
      this.data = this.data.map(function (item) {
        var newItem = _objectSpread({}, item);

        var _origin = newItem._origin;
        ...
        try {
          for (var _iterator2 = _origin[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
